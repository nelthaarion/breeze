...
func registerSecurity() {
	...
	// scripts or styles from a CDN, the default CSP will block them â€” re-run
...
func setupI18n(app *breeze.Breeze, router *breeze.Router) {
	...
	router.Use(middleware.LocaleMiddleware(I18n))
				...
				// block â€” otherwise the generated app fails at boot on the
				// log.Fatalf above.
...
// token it exists to hand out. Attach it per route instead â€” Handle takes
...
// unbounded paths or query strings â€” check middlewares/cache.go before putting
...
func setupEtag(app *breeze.Breeze, router *breeze.Router) {
	...
	router.Use(ETagCache.ETagMiddleware())
				...
				return featureOutput{Body: body, Notes: []string{
					"The ETag store is unbounded â€” no LRU or TTL. Keep it off routes with unbounded URLs.",
				}}, nil
			}
		},
	})
