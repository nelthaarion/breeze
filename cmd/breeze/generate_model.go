...
func generateModel(modulePath, name string, args []string) error {
	...
	b.WriteString("// Code generated by `breeze generate model` — edit freely.\n")
	...
	// ── the migration ────────────────────────────────────────────────────────
	if !*skipMigration {
		// nextMigrationVersion reads the directory, so it has to exist first —
		...
		notes = append(notes,
			"Review the id column — the portable type is a placeholder, not your engine's.",
			"Apply it:     breeze migrate up",
		)
		if !migratorPresent() {
			notes = append(notes, "Run `breeze add migrator --driver=postgres` — migrate needs a runner with your SQL driver.")
		}
	}
