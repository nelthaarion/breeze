...
//
// This is the one place a generatable setting is declared. Both input paths —
// `--config project.yaml` and `--section.field=value` CLI flags — resolve into
...
// unrelated to — and does not weaken — the rule that generated model accessors
...
//
// Type is a Go type expression as written in source — "string", "[]OrderItem",
// "map[string]*Address" — and is parsed and resolved at generation time so an
...
var (
	...
	// method delegates to httptransport — so generating it would produce a
...
// named elements is matched on the element's own Name (or Resource) value —
