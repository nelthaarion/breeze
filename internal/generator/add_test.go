...
// â€” `breeze add workerpool` just reports "unknown feature" for a subsystem the
// help text promised.
...
// block it just invalidated â€” the case that leaves a dashboard's Events page
...
// between two equally-distant names produced a different suggestion per run â€”
...
// TestUsageAdvertisesEveryFeature lived here until the generator moved out of
// package main. It cross-checks the CLI's usage text against this registry, and
// the usage text stayed behind with the command, so the test now lives beside
// it in cmd/breeze/usage_test.go, reading the registry through
// generator.FeatureNames().
