...
func generateView(modulePath, name string, args []string) error {
	...
	// which no generated block can reach — package scope is the requirement, not
	...
	if !hasBlock(featuresFileName, featureMarkerPrefix, "templates") {
		return fmt.Errorf("no package-scope template engine in this project — run `breeze add templates` first\n"+
