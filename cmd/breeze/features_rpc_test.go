...
// never reached — and the project compiles, starts, and answers JSON-RPC, so
...
// TestGeneratedRunErrorIsReported — a listener that failed to bind must say so.
...
// TestPoolOnlyWhenBlockingMethodsExist — the pool is what makes blocking
...
func TestPoolOnlyWhenBlockingMethodsExist(t *testing.T) {
	...
	if containsString(plain.Imports, runtimeImport) {
		t.Error("runtime was imported but not used — that does not compile")
	}
...
// TestBlockingMethodsUseRegisterBlocking — the whole point of declaring them.
...
// syntax error would be caught at generation time — but only for the block. The
...
// TestMaxMessageBytesOnlyWhenSet — zero means "keep the package default", and
...
// TestNoMethodsStillProducesAWorkingBlock — `add jsonrpc` with no methods is a
