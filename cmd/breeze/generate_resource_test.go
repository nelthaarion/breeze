...
// generateTestResourceFileWith runs the same two steps generateResource does —
// parse, then fill in defaults — so a test sees what a user would get rather
...
func TestResourceValidateTagsReachTheStruct(t *testing.T) {
	...
	// would reject age 0 and active false — a generated bug, not a convenience.
