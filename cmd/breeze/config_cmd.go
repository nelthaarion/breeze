...
// so; the config file supplies the settings that have nowhere else to live —
...
// repository — but its absence is not an error, because most projects do not
// have one.
...
// code. That is the worst kind of configuration bug — the file says one thing,
...
func unsupportedConfigKeys(cfg ProjectConfig) []string {
	...
	if cfg.WebSocket.Enabled && cfg.WebSocket.Rooms {
		unsupported = append(unsupported,
			"websocket.rooms — the generated handler has no room registry; use breeze.Rooms in your own code")
	}
	...
	// here — the user believes they have configured authentication.
	for _, m := range cfg.Middleware {
		if m.Secret != "" {
			unsupported = append(unsupported, fmt.Sprintf(
				"middleware.%s.secret — secrets are read from the environment, not generated into source; "+
	...
	if cfg.Fleet.Enabled && cfg.Fleet.Backend != "" && cfg.Fleet.Backend != "memory" {
		unsupported = append(unsupported,
			"fleet.backend — only the in-memory span store is generated; a persistent store is aggregator-side configuration")
	}
	return unsupported
}
