...
// identical ProjectConfig values — not merely both non-default — because that is
...
// TestUnknownFlagPathIsRejected — a mistyped path must not be silently ignored.
...
// TestUnknownYAMLKeyIsRejected — a mistyped key in a config file is the
...
// TestBareBooleanFlag — --websocket.enabled with no value means true.
...
// TestNonConfigArgsArePassedThrough — Load must not swallow the command's own
// arguments and flags.
...
// TestUnresolvedTypeInsideCompositeIsCaught — the reference must be resolved
...
// TestResolveModelOrderEmitsDependenciesFirst — a referenced model must be
...
// TestSharedDependencyGeneratedOnce — two models referencing the same type must
...
// TestResolveModelOrderIsDeterministic — output order must not depend on map
...
// TestSelfReferenceAndCyclesDoNotHang — mutually recursive models are legal Go
...
// URL does not crash — it silently exports over the fallback forever.
func TestFleetWSURLIsValidated(t *testing.T) {
	...
	// wss must be accepted, not just ws — refusing TLS would be worse than the
...
// TestValidateReportsEveryProblem — fixing a config file should not require
