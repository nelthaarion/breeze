...
// service rather than a sketch — the ordering constraint below (Fleet middleware
...
func buildFleetOutput(cfg FleetConfig, ctx featureCtx, gzip bool) (featureOutput, error) {
	...
	// wants — func(level, message, source string) — so with a dashboard block
...
func fleetTransportExpr(cfg FleetConfig, gzip bool) (expr, imp string, err error) {
	...
	case "http":
		...
		if gzip {
			...
			// JSON, but it costs CPU on the exporting service — so it stays
	...
	case "events":
		...
		// resolve to events.Default — the bus the framework already publishes
	...
	default:
		...
		return "", "", fmt.Errorf(
			"fleet transport %q has no generator — implemented: %s",
