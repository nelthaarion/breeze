...
// hub — a package-scope handle plus a setup function the dispatcher calls — but
...
// starts it in a goroutine, and the error is logged rather than dropped —
...
func registerJSONRPC() {
	register(&feature{
		...
		// matters — this listener is not in the HTTP middleware chain at all —
...
func validateRPCFeature(cfg JSONRPCConfig) error {
	...
	if cfg.Port == probe.Server.Port {
		return fmt.Errorf("--port %d is the default HTTP port — JSON-RPC needs its own listener and cannot share it", cfg.Port)
	}
	...
	// scaffold — or worse, silently not register it at all.
...
// I/O — a blocking handler on an event loop stalls every connection sharing it.
...
`)
	...
	fmt.Fprintf(&b, `	// Run blocks — it owns its own gnet listener, since JSON-RPC is a peer of
	...
	// they are code the developer is meant to edit — the same division `add`
	...
	if len(cfg.BlockingMethods) > 0 {
		...
		notes = append(notes, "No blocking methods, so no worker pool is created — use --blocking for handlers that do I/O.")
	}
