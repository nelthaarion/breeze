...
type field struct {
	...
	// Validate is the contents of the validate:"…" struct tag, empty when the
...
//
// The rules segment is optional and lands in the validate:"…" tag verbatim:
//
...
func parseFields(args []string) ([]field, error) {
	...
	for _, arg := range args {
		...
		if len(parts) < 2 || parts[0] == "" || parts[1] == "" {
			return nil, fmt.Errorf("invalid field %q — expected name:type[:rules] (e.g. email:string or email:string:required,email)", arg)
		}
		...
		if !supportedFieldTypes[typ] {
			return nil, fmt.Errorf("unsupported field type %q for %q — supported types: %s", typ, name, supportedFieldTypeList())
		}
...
// from a request — a model is filled by Scan, an event is constructed in
// process — and so rejects a rules segment instead of accepting one it would
// then drop.
//
...
func parseFieldsNoRules(kind string, args []string) ([]field, error) {
	for _, arg := range args {
		...
		if strings.Count(arg, ":") >= 2 {
			return nil, fmt.Errorf("field %q carries validation rules, which `generate %s` cannot use — only `generate resource` binds a request through binding.Bind, which is what reads a validate tag", arg, kind)
		}
	}
	...
	for _, part := range strings.Split(spec, ",") {
		...
		case validateRulesNeedingArg[ruleName]:
			if strings.TrimSpace(arg) == "" {
				return "", fmt.Errorf("rule %q on field %q needs an argument, as in %s=…", ruleName, fieldName, ruleName)
			}
		default:
			return "", fmt.Errorf("unknown validation rule %q on field %q — supported rules: email, max=N, min=N, oneof=a b c, required", ruleName, fieldName)
		}
...
// false — a generated bug rather than a generated convenience. Numeric and bool
...
// "email" becomes Email and "placed_at" becomes PlacedAt — a snake_case
...
// because a portable array type does not exist — Postgres has TEXT[], SQLite
// does not — and picking one would make the generated migration non-portable
