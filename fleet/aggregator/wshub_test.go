...
func TestWSAuthRolesAndPublishDecode(t *testing.T) {
	cfg := DefaultConfig()
	a := &Aggregator{
		cfg:      Config{Username: "viewer", Password: "read", IngestToken: "write"},
	...
	if !h.authenticate(viewer, wsEnvelope{Role: "viewer", Username: "viewer", Password: "read"}) ||
		viewer.role != wsViewer {
	...
	if !h.authenticate(ingest, wsEnvelope{Role: "ingest", Token: "write"}) ||
		ingest.role != wsIngest {
