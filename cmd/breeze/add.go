...
// are expected to edit — a handler, a workflow definition, an event struct —
...
func runAdd(args []string) error {
	...
	if len(positional) > 0 {
		return fmt.Errorf("`breeze add %s` takes no positional arguments, got %q — did you mean --%s?",
...
//
//   - Still pristine — the flags changed, or a sibling feature appeared and this
//     feature now composes with it. Replacing is exactly what the user asked
//     for, so it happens without ceremony.
//   - Edited since it was generated — refuse without --force. Discarding
...
func writeFeatureFiles(out featureOutput, force bool) (written, skipped []string, err error) {
	...
	// Sorted so the output is stable across runs — map iteration order is not.
...
func report(f *feature, out featureOutput, result blockResult, written, skipped []string) {
	...
	for _, path := range skipped {
		fmt.Printf("  kept  %s (already exists — use --force to overwrite)\n", path)
	}
...
// existed, so the bridge is absent — the project compiles, the dashboard serves,
...
func warnStaleDependents(added *feature, result blockResult) {
	...
	fmt.Printf("    Re-run those to pick it up — the blocks are untouched, so no --force is needed.\n")
}
...
// will not — and without it every add is silently inert: the code compiles,
...
// — and no edit distance connects those two words.
//
...
// else — real names and genuine mistakes alike — untouched for the caller to
// report.
//
...
// suggestionCandidates is every name worth matching a mistake against —
// canonical features plus their aliases — sorted so a tie between two
