...
// `breeze generate` — scaffold code you are then expected to edit.
//
...
type generator struct {
	...
	// identifier — true for everything that names a type or a handler group.
	NeedsName bool
...
func runGenerate(args []string) error {
	...
	if !ok {
		return fmt.Errorf("unknown generator %q — must be one of: %s",
...
Validation (resource only):
  ...
  Inferred:   a string field given no rules gets "required", and one named
              ...
              nothing — "required" means non-zero to the validator, so it
