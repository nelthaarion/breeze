...
// registration is order-independent — Router.Use rebuilds every route's chain
...
%s	}); err != nil {
		log.Fatalf("video: %%v", err)
				...
				if *signed {
					notes = append(notes,
						"Set VIDEO_SIGNING_SECRET — with signing on, an unsigned request is rejected.",
...
func registerWebSocket() {
	register(&feature{
		...
		Build: func(fs *flag.FlagSet) func(featureCtx) (featureOutput, error) {
			...
			// number of clients — fan out from a queue instead once that
			// number gets large.
		...
		Close: func(conn *breeze.WSConn, code uint16, reason string) {
			log.Printf("ws: %%s closed (%%d %%s)", conn.RemoteAddr(), code, reason)
				...
				notes := []string{
					...
					"payload aliases the read buffer, which is reused once the callback returns — copy anything you keep.",
...
func setupTemplates(app *breeze.Breeze, router *breeze.Router) {
	...
	router.View("/", Templates, "home", nil)
				...
				// generated setup treats that as fatal — so a layout and one
				...
				return featureOutput{
					...
					Notes: []string{
						...
						"DevMode reparses on every request — turn it off for production builds.",
					},
				}, nil
			}
		},
