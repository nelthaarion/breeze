...
// validates syntax and nothing else â€” a stub calling
...
//
// Keep this list exhaustive â€” TestE2EFeatureListIsComplete fails if a feature
...
// that change which declarations get emitted â€” --methods in particular, because
...
// inverse â€” an entry naming a feature that does not exist, which is how
...
func TestE2EFeatureListIsComplete(t *testing.T) {
	...
	for name, f := range features {
		...
		if !covered[name] {
			t.Errorf("feature %q (%s) is not covered by e2eFeatures â€” add it so the "+
...
//     failed with "applied migration -1 not found in migration files" â€” every
...
func TestEndToEndMigrateCycle(t *testing.T) {
	...
	// and an in-memory database would not survive between them â€” every command
	...
	// Rolling back everything must leave the ledger clean enough to re-apply â€”
...
//
// What must hold is that the passes converge and then stay put â€” a block that
...
// â€” the whole-project test can only report that some block moved, whereas this
// names the feature.
//
...
// that block"), and before blocks carried a checksum it failed â€” a block whose
...
func TestAddReplacesWhenFlagsChange(t *testing.T) {
	...
	if strings.Contains(string(content), `"/admin"`) {
		t.Errorf("the old basepath survived in %s â€” the block was not replaced", featuresFileName)
	}
}
...
// â€” it compiles, it serves, the page renders â€” indicates why.
