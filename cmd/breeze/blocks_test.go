...
// generated from. Both answers must be "unchanged" and "pristine" — before the
...
//
// The mangling below is deliberately worse than anything gofmt produces — a
// top-level func pushed off column 0 — because the point is that the checksum
...
func TestUnstampedBlockIsNotPristine(t *testing.T) {
	...
	// The body must still be readable and comparable — that path is shared with
	// the stamped case.
	if !sameBlockBody(stored, testBlockBody) {
...
// regression here would append a second block rather than replace the first —
