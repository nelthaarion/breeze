...
func TestAggregatorConsumesSignedInProcessEvents(t *testing.T) {
	...
	transport := eventtransport.New(
		eventtransport.Config{Bus: bus, IngestToken: "secret", ServiceName: "orders"},
	)
	span := fleet.Span{
	...
	if err := transport.ExportHeartbeat(
		context.Background(),
		"",
		fleet.Heartbeat{Service: "orders", InstanceID: "one"},
	); err != nil {
		t.Fatal(err)
	}
	...
	t.Fatalf(
		"events not consumed: stats=%+v registry=%+v",
		a.Store().Stats(),
		a.Registry().Snapshot(time.Now()),
	)
}
