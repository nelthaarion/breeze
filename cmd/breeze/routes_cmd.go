...
// `breeze routes` — list the project's generated routes.
//
...
// that does not currently compile — which is when you most want to see what is
// registered — at the cost of only knowing about routes in that file. Routes
...
func parseRoutes(fileName string) ([]routeEntry, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("%s not found — run this from a project root, after `breeze generate`", fileName)
		}
		return nil, err
	}
...
func printRouteTable(routes []routeEntry, fileName string) {
	...
	fmt.Printf("\n%d route(s) in %s.\n", len(routes), fileName)
	fmt.Printf("Routes registered outside this file — by hand in main.go, or mounted by a\n" +
		"feature at runtime (dashboard, video, docs) — are not listed.\n")
}
