...
// covers the part that was missing until now — that a configuration file
...
// TestExtractConfigFlagRejectsMissingValue — a bare --config is a mistake, and
...
// TestMissingExplicitConfigIsAnError — naming a file that is not there must
...
// TestDefaultConfigFileIsPickedUp — a breeze.yaml in the working directory is
...
// TestConfigFeatureNamesResolvesAliases — a hyphenated name in YAML has to
...
// — including one whose file says nothing but `module:`. That may well be
...
// TestTypedSectionsEnableTheirFeature — a section with enabled: true should not
...
// wrong when this was first written — --path for docs, which takes --ui-path;
...
func TestConfigFlagsForAreAcceptedByTheirFeature(t *testing.T) {
	...
	for _, name := range configFeatureNames(cfg) {
		t.Run(name, func(t *testing.T) {
			...
			// An empty list is legitimate — most features have no config
...
// TestUnsupportedConfigKeysAreReported — a key that parses, validates, and then
...
// TestNewWithFlagOverridingConfig — the documented precedence, through the
...
// TestNewRejectsInvalidConfig — validation has to run before anything is
...
// TestApplyConfigFeaturesIsIdempotent — applying the same file twice must
