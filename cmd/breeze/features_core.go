...
// handles — EventBus, ObsCollector, DashboardCollector, WorkflowEngine — that
	...
	register(&feature{
		...
		Build: func(fs *flag.FlagSet) func(featureCtx) (featureOutput, error) {
			...
			return func(ctx featureCtx) (featureOutput, error) {
				...
				// DisableMetrics — Config.normalize resolves the pair as
...
//
// The framework publishes here too — requests, WebSocket lifecycle, workflow
// steps — so keeping one bus for your own events means every subscriber and
...
` + cfg.String() + `	})
				...
				if *async {
					notes = append(notes, "Async dispatch means Emit returns before listeners finish — use EmitAsyncWaitBus where you need to observe their errors.")
				}
...
// connections, jobs and health checks on it — RegisterConnection,
// RegisterJob, RegisterTask, RegisterHealthCheck, SetDBInspector — and they
...
%s
	...
	// measures, so it must be registered before they run — which is what this
	...
	router.Use(dashboard.Middleware(DashboardCollector))%s
				...
				switch {
				case *noAuth:
					notes = append(notes, "Auth is disabled — do not expose this build publicly.")
...
%s	})
	...
	// malformed definition — a duplicate step name, a dependency on a step
	// that does not exist — so it is worth failing loudly at boot:
	//
...
func ShutdownWorkflow() {
	...
	if err := WorkflowEngine.Shutdown(ctx); err != nil {
		log.Printf("workflow: shutdown: %%v", err)
				...
				notes := []string{
					"Call ShutdownWorkflow() on your shutdown path — in-flight runs are dropped otherwise.",
...
func registerTuning() {
	...
	// loop for any handler that does I/O — a database call, an outbound HTTP
	...
	app.SetZeroCopyHeaders(%t)
				...
				notes := []string{
					"Worker-pool size is set in main.go where NewEventLoopWorkerPool is called — it is passed to breeze.New, so it cannot be changed from here.",
				}
				if *inline {
...
// runner is where one gets chosen — and it lands in the user's go.mod, not
// breeze's.
	...
	register(&feature{
		...
		Build: func(fs *flag.FlagSet) func(featureCtx) (featureOutput, error) {
			...
			return func(ctx featureCtx) (featureOutput, error) {
				...
				if !ok {
					return featureOutput{}, fmt.Errorf("unknown driver %q — must be one of: %s",
				...
				return featureOutput{
					...
					Notes: []string{
						...
						"`breeze migrate` shells out to this binary — it is what makes those subcommands work at all, since the CLI itself has no driver compiled in.",
					},
				}, nil
			}
		},
...
func migratorRunner(driverImport, driverName string) string {
	const tmpl = `// Code generated by ` + "`breeze add migrator`" + `. Safe to edit — the CLI will not
// rewrite this file.
//
...
// driver — pulling in Postgres, MySQL and SQLite so the CLI can open any of
// them would put all three in every project's dependency graph — so the
	...
	case "status":
		...
		for _, e := range entries {
			...
			if e.ChecksumMismatch {
				mark += "  !! checksum mismatch — file edited after apply"
			}
