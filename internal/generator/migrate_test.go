...
// exist â€” so `generate model` recommended `breeze add migrator` to people who
// had already run it.
func TestMigratorPresentTracksTheRunner(t *testing.T) {
	...
	// The directory alone is not a runner â€” `add migrator` creates migrations/
...
func TestStandaloneFeaturesHaveNoBlock(t *testing.T) {
	...
	for name, f := range features {
		...
		if hasBlock(featuresFileName, featureMarkerPrefix, name) {
			t.Errorf("standalone feature %q wrote a block to %s â€” it has no dispatcher call, "+
