...
func checkProtocolVersion(version string) error {
	...
	return fmt.Errorf("MCP-Protocol-Version %q is not supported; this server speaks %s", version, supportedVersionList())
}

// validateModernRequestHeaders enforces SEP-2243's body/header agreement.
// It deliberately validates only the standard routing headers that this server
// advertises; per-tool Mcp-Param-* headers are not generated by any tool schema
// today and therefore cannot be safely mirrored.
func validateModernRequestHeaders(h http.Header, body []byte) error {
	p, ok := probeRequest(body)
	if !ok {
		return fmt.Errorf("2026-07-28 requests must contain one JSON-RPC object")
	}
	method := strings.TrimSpace(h.Get(methodHeader))
	if method == "" {
		return fmt.Errorf("%s is required for 2026-07-28 requests", methodHeader)
	}
	if method != p.Method {
		return fmt.Errorf("%s %q does not match JSON-RPC method %q", methodHeader, method, p.Method)
	}
	if p.Method == "initialize" || p.Method == "notifications/initialized" {
		return fmt.Errorf("%s is not supported by MCP %s", p.Method, modernProtocol)
	}
	switch p.Method {
	case "tools/call":
		name := strings.TrimSpace(h.Get(nameHeader))
		if name == "" {
			return fmt.Errorf("%s is required for tools/call", nameHeader)
		}
		if name != p.Params.Name {
			return fmt.Errorf("%s %q does not match tool name %q", nameHeader, name, p.Params.Name)
		}
	default:
		if name := strings.TrimSpace(h.Get(nameHeader)); name != "" {
			return fmt.Errorf("%s must be absent for %s", nameHeader, p.Method)
		}
	}
	return nil
}

// probeRequest reads only the fields needed by the modern transport validator.
...
func probeRequest(body []byte) (modernProbe, bool) {
	var p modernProbe
	if err := json.Unmarshal(body, &p); err != nil || p.Method == "" {
		return modernProbe{}, false
	}
	return p, true
}
...
func writeRPCError(w http.ResponseWriter, status int, message string) {
	writeRPCErrorCode(w, status, codeTransport, message)
}

func writeRPCErrorCode(w http.ResponseWriter, status, code int, message string) {
