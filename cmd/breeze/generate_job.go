...
func generateJob(modulePath, name string, args []string) error {
	...
	b.WriteString("// Code generated by `breeze generate job` — edit freely.\n")
	b.WriteString("package jobs\n\n")
	b.WriteString("import (\n")
	b.WriteString("\t\"context\"\n")
	b.WriteString("\t\"log\"\n")
	b.WriteString("\t\"time\"\n\n")
	b.WriteString("\t\"github.com/nelthaarion/breeze/dashboard\"\n")
	b.WriteString(")\n\n")
	...
	b.WriteString("\t// valid and means \"do not report\" — the job still runs.\n")
	...
	// and update — there is no separate UpdateTask to call.
	...
	path := filepath.Join("jobs", fileSlug(name)+".go")
	...
	fmt.Fprintf(&body, "var %sJob = jobs.New%s()\n\n", lower, name)
	...
	if hasDashboard {
		...
		body.WriteString("\t" + lower + "Job.Reporter = DashboardCollector\n\n")
	}
	...
	notes := []string{
		fmt.Sprintf("Implement:    jobs.%s.Run in %s", name, path),
	...
	notes = append(notes,
		"Started with context.Background() — swap in a cancellable context to stop it on shutdown.")
	printNotes(notes)
	return nil
}
