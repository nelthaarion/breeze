...
//
// which names the model, the field and the type — the three things needed to
...
func (c *ProjectConfig) validateFleet() []string {
	...
	case !contains(fleetImplementedTransports, c.Fleet.Transport):
		errs = append(errs, fmt.Sprintf(
			"fleet.transport %q is defined by the Fleet specification but has no transport implementation in the fleet package yet, so it cannot be generated — currently generatable: %s",
	...
	if !contains(fleetBackends, c.Fleet.Backend) {
		...
		errs = append(errs, fmt.Sprintf(
			"fleet.backend %q has no implementation in the events package yet — currently generatable: %s",
	...
	// cannot. A wrong scheme here would therefore not fail loudly at startup —
...
func (c *ProjectConfig) validateJSONRPC() []string {
	...
	if c.JSONRPC.Port == c.Server.Port {
		errs = append(errs, fmt.Sprintf(
			"jsonrpc.port %d is the same as server.port — JSON-RPC listens on its own gnet listener and cannot share the HTTP port",
			c.JSONRPC.Port))
	}
	...
	for _, m := range c.JSONRPC.Methods {
		...
		// The spec reserves the rpc. prefix for internal methods (§4).
		if strings.HasPrefix(m, "rpc.") {
			errs = append(errs, fmt.Sprintf(
				"jsonrpc method %q uses the rpc. prefix, which JSON-RPC 2.0 §4 reserves for internal methods", m))
		}
		if seen[m] {
	...
	// scaffolded, and the project would not compile — or, if the name merely
...
//
// The composite forms are peeled off recursively — *T, []T, map[K]V and any
// nesting of them — leaving the identifiers that have to resolve. Returning the
...
// an error — Go handles mutually recursive struct types through pointers, and
// refusing them would reject legal models — so a cycle is emitted in name order
// instead of failing.
